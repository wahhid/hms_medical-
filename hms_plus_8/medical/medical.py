...
class physician (osv.osv):
	...
	_columns = {
		...
		'available_lines': fields.one2many('medical.physician.line', 'physician_id', 'Physician Availability'),
		'info' : fields.text ('Extra info'),
		}
...
class physician_line (osv.osv):
	...
	_columns = {
		...
		'physician_id': fields.many2one('medical.physician', 'Physician',select=True,ondelete='cascade'),		
		}
physician_line ()
	...
	def _patient_age(self, cr, uid, ids, name, arg, context={}):
		def compute_age_from_dates (patient_dob,patient_deceased,patient_dod):
			...
			if (patient_dob):
				...
				#years_months_days = str(delta.d ) +"y "+ str(delta.months) +"m "+ str(delta.days)+"d" + deceased
...
class appointment (osv.osv):
	...
	_columns = {
		...
		'appointment_date1' : fields.date('Appointment Date'),
	...
	_order = "appointment_date1 desc"
	_defaults = {
           'urgency': lambda *a: 'a',
	       'name': lambda obj, cr, uid, context: '/',
	       'appointment_date1': fields.date.context_today, 
        	}
	
	def create(self, cr, uid, vals, context=None):
		if vals.get('name','/')=='/':
			vals['name'] = self.pool.get('ir.sequence').get(cr, uid, 'medical.appointment') or '/'
		if vals.get('doctor') and vals.get('appointment_date1') and vals.get('appointment_time'):
			self.onchange_partnerid(cr,uid,vals,vals.get('doctor'),vals.get('appointment_date1'),vals.get('appointment_time'))
		return super(appointment, self).create(cr, uid, vals, context=context)
	
	def onchange_partnerid(self, cr, uid, ids, partner,appointment_date1,appointment_time,context=None):
		partner_speciality = ""
		patient_line_obj = self.pool.get('medical.physician.line')
		v = ''
		selected_day = ''
		res = {}
		if not partner:
			return {}
		if partner:			
			if appointment_date1 and appointment_time:
				selected_day = datetime.datetime.strptime(appointment_date1, '%Y-%m-%d').strftime('%A')				
				#raise osv.except_osv(_('Time'), _(str(t)))
				if selected_day:
					avail_day = patient_line_obj.search(cr, uid, [('name', '=', str(selected_day)),('physician_id', '=', partner)], context=context)
					if not avail_day:
						raise osv.except_osv(_('Not Available'), _('Physician is not available on selected day'))					
					else:
						for ad in patient_line_obj.browse(cr, uid, avail_day, context=context): 
							app_time = self.get_time_string(appointment_time).split(':')
							start_time = self.get_time_string(ad.start_time).split(':')
							end_time = self.get_time_string(ad.end_time).split(':')
							
							t1 = datetime.time(int(start_time[0]),int(start_time[1]),0)							
							t2 = datetime.time(int(app_time[0]),int(app_time[1]),0)
							t3 = datetime.time(int(end_time[0]),int(end_time[1]),0)
							#raise osv.except_osv(_('Time'), _(str(t3)))
							
							if not (t2 > t1 and t2 < t3):						
								raise osv.except_osv(_('Not Available'), _('Physician is not available on selected time'))
							else:
								final_date = str(appointment_date1)+' '+str(t2)
								#appoint_date = datetime.datetime.strptime(final_date,'%Y-%m-%d %H:%M:%S')	
								#raise osv.except_osv(_('Time'), _(str(appoint_date)))
								# Fetch Speciality information
								cr.execute ("select speciality from medical_physician where id=%s",(partner,))
								values = cr.fetchone()
								partner_speciality = ''
								if values:
					 				if values[0]: 			 	
					 					partner_speciality = str(values[0])				
					 			v = {'speciality':partner_speciality}		
								res = {'value': v}
			else:
				raise osv.except_osv(_('Warning'), _('Please first properly enter both availability date and time'))
		return res
